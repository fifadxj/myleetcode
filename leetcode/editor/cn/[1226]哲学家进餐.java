...
class DiningPhilosophers {//0号哲学家在0号叉子右边
    List<Lock> forkLocks = new ArrayList<>();
    ...
    public void wantsToEat(int philosopher,
        ...
        int lefForkIndex = leftForkIndex(philosopher);
        int rightForkIndex = rightForkIndex(philosopher);
        Lock leftForkLock = forkLocks.get(lefForkIndex);
        Lock rightForkLock = forkLocks.get(rightForkIndex);

        if (lefForkIndex < rightForkIndex) {
            leftForkLock.lock();
            rightForkLock.lock();
        } else {
            rightForkLock.lock();
    ...
    private int leftForkIndex(int philosopher) {
        return philosopher;
    }

    private int rightForkIndex(int philosopher) {
