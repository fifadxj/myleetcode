...
class Solution {
    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        for (Map.Entry<Integer, List<Data>> entry : map.entrySet()) {
            ...
            values.sort((d1, d2) -> {//must d1.col == d2.col
                if (d1.row == d2.row) {
            ...
            ans.add(values.stream().map(o -> o.value).collect(Collectors.toList()));
        }
    ...
    void dfs(TreeNode node, int row, int col) {
        ...
        List<Data> values = map.getOrDefault(col, new ArrayList());
        values.add(new Data(node.val, row, col));
        map.put(col, values);
    ...
    class Data {
        ...
        int col;
        int value;
        Data(int value, int row, int col) {
            this.value = value;
            this.row = row;
            this.col = col;
        }
