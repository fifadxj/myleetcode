...
class Solution {
    public boolean checkMove(char[][] board, int rMove, int cMove, char color) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int rDirection = direction[0];
            int cDirection = direction[1];
            int curRow = rMove + rDirection;
            int curCol = cMove + cDirection;
            if (curRow >= 0 && curRow < 8 && curCol >= 0 && curCol < 8 && board[curRow][curCol] == midColor) {
                for (int row = curRow + rDirection, col = curCol + cDirection; row >= 0 && row < 8 && col >= 0 && col < 8;  row = row + rDirection, col = col + cDirection) {
                    if (board[row][col] == '.') {
                        break;
                    }
